...
export default function EditMotorcyclePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (motorError) {
        setError('Gagal memuat data motor: ' + motorError.message);
      } else if (motorData) {
        setFormData({
          id: motorData.id,
          brand_id: motorData.brand_id,
          model_code: motorData.model_code,
          name: motorData.name,
          latest_price: motorData.latest_price.toString(),
          compression_ratio: motorData.compression_ratio,
          engine_type: motorData.engine_type,
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Edit Motor</h1>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-8">
        Rekomendasi oli dan bensin (Knowledge Base) akan dihitung ulang secara otomatis jika Anda mengubah kategori atau rasio kompresi.
      </p>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {/* ID */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Motorcycle ID</label>
            <input type="text" name="id" required value={formData.id} disabled className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400 outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-70" />
          </div>
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-gray-800 flex justify-end gap-3">
          ...
          <button type="submit" disabled={isSubmitting} className="inline-flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-70">
